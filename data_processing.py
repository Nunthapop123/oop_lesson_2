...
players_filtered = players.filter(lambda x: 'ia' in x['team']).filter(lambda x: int(x['minutes']) < 200).filter(lambda x: int(x['passes']) > 100)
players_selected = players_filtered.select(['surname', 'team', 'position'])
print(players_selected)
teams = my_DB.search('teams')
high_teams = teams.filter(lambda x: int(x['ranking'])<10)
print('The average number of games played for teams ranking below 10:',high_teams.aggregate(lambda x: sum(x)/len(x),'games'))
low_teams = teams.filter(lambda x: int(x['ranking'])>10)
print('The average number of games played for teams ranking above 10:',low_teams.aggregate(lambda x: sum(x)/len(x),'games'))
players_forward = players.filter(lambda x: x['position'] == 'forward')
print('The average number of passes made by forwards:',players_forward.aggregate(lambda x: sum(x)/len(x),'passes'))
players_midfield = players.filter(lambda x: x['position'] == 'midfielder')
print('The average number of passes made by midfielders:',players_midfield.aggregate(lambda x: sum(x)/len(x),'passes'))
print()
...
print()
passenger_info = titanic.filter(lambda x: x['gender'] == 'M').filter(lambda x: x['embarked'] == 'Southampton')
total_number = len(passenger_info.table)
print('The total number of male passengers embarked at Southampton:', total_number)
